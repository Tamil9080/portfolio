...
export default function Achievements() {
  ...
  return (
    <section ref={ref} id="achievements" className="py-32 px-6 max-w-7xl mx-auto relative scroll-mt-24">
        ...
        <div className={`flex flex-wrap gap-2 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          {categories.map((cat) => (
            <button
              ...
              className={`flex items-center gap-3 px-6 py-3 rounded-xl font-black italic font-[Anton] uppercase tracking-widest text-[10px] transition-all border active:scale-[0.95] ${
                activeCategory === cat.id 
                ? 'bg-red-600 text-white border-red-600 shadow-[0_0_25px_rgba(220,38,38,0.4)]' 
                : 'bg-white/5 text-white/50 border-white/5 hover:border-red-600/30'
              }`}
            >
              <span className={activeCategory === cat.id ? 'text-white animate-pulse' : 'group-hover:text-red-500 transition-colors'}>{cat.icon}</span>
              {cat.label}
            </button>
          ))}
        </div>
        ...
        {achievements
          ...
          .map((item, i) => {
            ...
            return (
              ...
              >
                <div className="p-[1px] rounded-2xl bg-white/5 border border-white/5 transition-all overflow-hidden shadow-xl">
                  <div className="bg-[#0a0a0c]/90 backdrop-blur-md rounded-[0.95rem] p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                    <div className="flex items-center gap-6 relative z-10">
                      <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-xl group-hover:scale-110 transition-all duration-500 group-hover:border-white/20">
                        <span className="text-white transition-colors"><Icon /></span>
                      </div>
                      <div>
                        <h3 className="text-xl font-black italic font-[Anton] text-white transition-all uppercase tracking-wide drop-shadow-[0_2px_2px_rgba(0,0,0,0.5)]">
                          {item.title}
                        </h3>
                        <p className="text-gray-400 text-[11px] mt-1 font-bold uppercase tracking-wide opacity-80">{item.description}</p>
                      </div>
                    </div>

                    <div className="relative z-10 flex flex-col items-end">
                      <div className="text-xs font-black text-white/30 tracking-[0.1em]">{item.year}</div>
                      <div className="mt-1 px-2 py-0.5 bg-white/5 rounded-full border border-white/10 text-[7px] font-black text-white/10 uppercase tracking-[0.2em]">
                        M_ENTRY_LOG_{i+1}
                      </div>
                    </div>
                  </div>
